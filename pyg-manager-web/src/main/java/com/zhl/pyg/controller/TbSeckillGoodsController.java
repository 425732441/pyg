...
public class TbSeckillGoodsController {
     ...
     */
    @DubboReference
    private TbSeckillGoodsService tbSeckillGoodsServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbSeckillGoods result = tbSeckillGoodsServiceImpl.selectById(id);
     ...
     * 新增一条数据
     *
     * @param tbSeckillGoods 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbSeckillGoods tbSeckillGoods) {
        int result = tbSeckillGoodsServiceImpl.insert(tbSeckillGoods);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbSeckillGoods tbSeckillGoods) {
        TbSeckillGoods result = tbSeckillGoodsServiceImpl.update(tbSeckillGoods);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbSeckillGoodsServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbSeckillGoods>> selectAll() {
        List<TbSeckillGoods> tbSeckillGoodss = tbSeckillGoodsServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbSeckillGoodss)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbSeckillGoodss);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbSeckillGoods> page = tbSeckillGoodsServiceImpl.selectPage(current, size);
