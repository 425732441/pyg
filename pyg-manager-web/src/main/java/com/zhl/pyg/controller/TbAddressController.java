...
public class TbAddressController {
     ...
     */
    @DubboReference
    private TbAddressService tbAddressServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbAddress result = tbAddressServiceImpl.selectById(id);
    ...
    public Result insert(@RequestBody TbAddress tbAddress) {
        int result = tbAddressServiceImpl.insert(tbAddress);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbAddress tbAddress) {
        TbAddress result = tbAddressServiceImpl.update(tbAddress);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbAddressServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbAddress>> selectAll() {
        List<TbAddress> tbAddresss = tbAddressServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbAddresss)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbAddresss);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbAddress> page = tbAddressServiceImpl.selectPage(current, size);
