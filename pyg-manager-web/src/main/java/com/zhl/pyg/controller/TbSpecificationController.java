...
public class TbSpecificationController {
     ...
     */
    @DubboReference
    private TbSpecificationService tbSpecificationServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbSpecification result = tbSpecificationServiceImpl.selectById(id);
     ...
     * 新增一条数据
     *
     * @param tbSpecification 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbSpecification tbSpecification) {
        int result = tbSpecificationServiceImpl.insert(tbSpecification);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbSpecification tbSpecification) {
        TbSpecification result = tbSpecificationServiceImpl.update(tbSpecification);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbSpecificationServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbSpecification>> selectAll() {
        List<TbSpecification> tbSpecifications = tbSpecificationServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbSpecifications)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbSpecifications);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbSpecification> page = tbSpecificationServiceImpl.selectPage(current, size);
