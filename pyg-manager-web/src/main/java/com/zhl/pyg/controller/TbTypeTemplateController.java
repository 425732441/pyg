...
public class TbTypeTemplateController {
     ...
     */
    @DubboReference
    private TbTypeTemplateService tbTypeTemplateServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbTypeTemplate result = tbTypeTemplateServiceImpl.selectById(id);
     ...
     * 新增一条数据
     *
     * @param tbTypeTemplate 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbTypeTemplate tbTypeTemplate) {
        int result = tbTypeTemplateServiceImpl.insert(tbTypeTemplate);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbTypeTemplate tbTypeTemplate) {
        TbTypeTemplate result = tbTypeTemplateServiceImpl.update(tbTypeTemplate);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbTypeTemplateServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbTypeTemplate>> selectAll() {
        List<TbTypeTemplate> tbTypeTemplates = tbTypeTemplateServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbTypeTemplates)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbTypeTemplates);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbTypeTemplate> page = tbTypeTemplateServiceImpl.selectPage(current, size);
