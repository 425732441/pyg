...
public class TbGoodsDescController {
     ...
     */
    @DubboReference
    private TbGoodsDescService tbGoodsDescServiceImpl;
    ...
    public Result selectOne(@PathVariable("goodsId") Long goodsId) {
        TbGoodsDesc result = tbGoodsDescServiceImpl.selectById(goodsId);
     ...
     * 新增一条数据
     *
     * @param tbGoodsDesc 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbGoodsDesc tbGoodsDesc) {
        int result = tbGoodsDescServiceImpl.insert(tbGoodsDesc);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbGoodsDesc tbGoodsDesc) {
        TbGoodsDesc result = tbGoodsDescServiceImpl.update(tbGoodsDesc);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("goodsId") Long goodsId) {
        int result = tbGoodsDescServiceImpl.deleteById(goodsId);
        if (result > 0) {
    ...
    public Result<List<TbGoodsDesc>> selectAll() {
        List<TbGoodsDesc> tbGoodsDescs = tbGoodsDescServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbGoodsDescs)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbGoodsDescs);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbGoodsDesc> page = tbGoodsDescServiceImpl.selectPage(current, size);
