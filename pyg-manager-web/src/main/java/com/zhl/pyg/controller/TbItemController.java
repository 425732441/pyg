...
public class TbItemController {
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbItem result = tbItemServiceImpl.selectById(id);
    ...
    public Result insert(@RequestBody TbItem tbItem) {
        int result = tbItemServiceImpl.insert(tbItem);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbItem tbItem) {
        TbItem result = tbItemServiceImpl.update(tbItem);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbItemServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbItem>> selectAll() {
        List<TbItem> tbItems = tbItemServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbItems)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbItems);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbItem> page = tbItemServiceImpl.selectPage(current, size);
