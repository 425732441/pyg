...
public class TbFreightTemplateController {
     ...
     */
    @DubboReference
    private TbFreightTemplateService tbFreightTemplateServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Long id) {
        TbFreightTemplate result = tbFreightTemplateServiceImpl.selectById(id);
     ...
     * 新增一条数据
     *
     * @param tbFreightTemplate 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbFreightTemplate tbFreightTemplate) {
        int result = tbFreightTemplateServiceImpl.insert(tbFreightTemplate);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbFreightTemplate tbFreightTemplate) {
        TbFreightTemplate result = tbFreightTemplateServiceImpl.update(tbFreightTemplate);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Long id) {
        int result = tbFreightTemplateServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbFreightTemplate>> selectAll() {
        List<TbFreightTemplate> tbFreightTemplates = tbFreightTemplateServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbFreightTemplates)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbFreightTemplates);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbFreightTemplate> page = tbFreightTemplateServiceImpl.selectPage(current, size);
