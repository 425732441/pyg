...
public class TbCitiesController {
     ...
     */
    @DubboReference
    private TbCitiesService tbCitiesServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Integer id) {
        TbCities result = tbCitiesServiceImpl.selectById(id);
    ...
    public Result insert(@RequestBody TbCities tbCities) {
        int result = tbCitiesServiceImpl.insert(tbCities);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbCities tbCities) {
        TbCities result = tbCitiesServiceImpl.update(tbCities);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Integer id) {
        int result = tbCitiesServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbCities>> selectAll() {
        List<TbCities> tbCitiess = tbCitiesServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbCitiess)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbCitiess);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbCities> page = tbCitiesServiceImpl.selectPage(current, size);
