...
public class TbProvincesController {
     ...
     */
    @DubboReference
    private TbProvincesService tbProvincesServiceImpl;
    ...
    public Result selectOne(@PathVariable("id") Integer id) {
        TbProvinces result = tbProvincesServiceImpl.selectById(id);
     ...
     * 新增一条数据
     *
     * @param tbProvinces 实体类
     * @return Result对象
     */
    @PostMapping(value = "/insert")
    public Result insert(@RequestBody TbProvinces tbProvinces) {
        int result = tbProvincesServiceImpl.insert(tbProvinces);
        if (result > 0) {
    ...
    public Result update(@RequestBody TbProvinces tbProvinces) {
        TbProvinces result = tbProvincesServiceImpl.update(tbProvinces);
        if (Objects.nonNull(result)) {
    ...
    public Result delete(@PathVariable("id") Integer id) {
        int result = tbProvincesServiceImpl.deleteById(id);
        if (result > 0) {
    ...
    public Result<List<TbProvinces>> selectAll() {
        List<TbProvinces> tbProvincess = tbProvincesServiceImpl.selectAll();
        if (CollectionUtils.isEmpty(tbProvincess)) {
            return new Result<>(true, StatusCode.ERROR, "查询全部数据失败");
        }
        return new Result<>(true, StatusCode.OK, "查询全部数据成功", tbProvincess);
    ...
    public Result selectPage(@PathVariable("current") Integer current, @PathVariable("size") Integer size) {
        IPage<TbProvinces> page = tbProvincesServiceImpl.selectPage(current, size);
